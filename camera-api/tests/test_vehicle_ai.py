...
from app.jobs import vehicle_ai
from app.jobs.vehicle_ai import (
    VEHICLE_MODULE_CODE,
    _recently_flagged,
    process_camera_frame_pair_for_vehicle,
    run_vehicle_ai_sweep_once,
)
from app.models import Building, Camera, Event
from app.services.object_detection import DetectedObject
from tests.conftest import TestSessionLocal

_CAR_DETECTION = [DetectedObject(class_id=2, class_name="car", confidence=0.75, bbox=(5.0, 5.0, 120.0, 80.0))]
...
async def a_camera(db_session, seeded):
    ...
    camera = Camera(
        name="Hovli kamerasi", ip="10.0.9.10", building_id=building.id,
...
class TestRecentlyFlagged:
    ...
    async def test_recent_event_at_same_camera_is_flagged(self, db_session, a_camera):
        db_session.add(Event(
            ...
            module_code=VEHICLE_MODULE_CODE, module_name="Transport", group="F",
            confidence=75, severity="o'rta", status="yangi",
        ))
        await db_session.commit()
...
@pytest.mark.usefixtures("seeded")
class TestProcessCameraFramePairForVehicle:
    async def test_no_vehicle_raises_no_event(self, db_session, a_camera, monkeypatch):
        async def fake_detect_objects(frame_bytes, class_ids, confidence=0.5):
            return []

        monkeypatch.setattr(vehicle_ai, "detect_objects", fake_detect_objects)
        raised = await process_camera_frame_pair_for_vehicle(b"a", b"b", db_session, a_camera)
        assert raised is False

    async def test_vehicle_only_in_second_frame_is_not_confirmed(self, db_session, a_camera, monkeypatch):
        call_count = {"n": 0}

        async def fake_detect_objects(frame_bytes, class_ids, confidence=0.5):
            call_count["n"] += 1
            if call_count["n"] == 1:
                return _CAR_DETECTION
            return []
    ...
    async def test_vehicle_in_both_frames_raises_an_event(self, db_session, a_camera, monkeypatch):
        async def fake_detect_objects(frame_bytes, class_ids, confidence=0.5):
            return _CAR_DETECTION
        ...
        events = (await db_session.execute(select(Event))).scalars().all()
        assert len(events) == 1
        assert events[0].module_code == VEHICLE_MODULE_CODE
        assert events[0].camera_name == a_camera.name
        assert events[0].confidence == 75
    ...
    async def test_same_camera_within_dedup_window_is_not_reraised(self, db_session, a_camera, monkeypatch):
        ...
        monkeypatch.setattr(vehicle_ai, "detect_objects", fake_detect_objects)
        first = await process_camera_frame_pair_for_vehicle(b"a", b"b", db_session, a_camera)
        second = await process_camera_frame_pair_for_vehicle(b"a", b"b", db_session, a_camera)
...
class TestSweepConcurrency:
    async def test_one_camera_failing_does_not_stop_the_others(self, db_session, seeded, monkeypatch):
        ...
        monkeypatch.setattr(vehicle_ai, "grab_frame_pair_for_camera", flaky_grab_frame_pair)
        monkeypatch.setattr(vehicle_ai, "detect_objects", fake_detect_objects)
